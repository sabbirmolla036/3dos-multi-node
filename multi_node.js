...
**How it works:**
- Place your main account in `main_account.txt` (first line: `email:password`).
- Place proxies in `proxies.txt` (one per line, or leave empty for no proxy).
- Run with `node multi_node.js`.
- Enter how many nodes to run (up to number of proxies or 100 if no proxies).
- Each node logs in and performs check-in every 12 hours.

**You can adapt the `runNode` function to add more actions as needed.**

Let me know if you want a version for multiple accounts or with more advanced features!
